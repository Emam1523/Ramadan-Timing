...
function getDistrictElement() {
  return document.getElementById("district");
}
...
async function getPrayerTimes(district) {
  ...
  if (pastOrToday.length > 0) {
    const lastEntry = pastOrToday[pastOrToday.length - 1];

    // Check if current date is after the last Ramadan date
    if (lastEntry.parsed.getTime() < targetDate.getTime()) {
      // Ramadan has finished
      return { isFinished: true, lastDate: lastEntry.item.date };
    }

    // Current date matches a Ramadan date
...
async function applyDistrict(districtName) {
  ...
  // Handle finished Ramadan case
  if (prayerData && prayerData.isFinished) {
    hideTurnOnLocationButton();
    showDistrictSelector(null); // Ensure district selector is visible/state is clean
    ...
    // Set a specialized message
    setLocationNotice(
      `Ramadan has finished. The last day of Ramadan was on ${prayerData.lastDate}.`,
    );
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          await resolveAndApplyFromCoords(pos.coords);
        } catch (e) {
          console.error(e);
        }
      },
